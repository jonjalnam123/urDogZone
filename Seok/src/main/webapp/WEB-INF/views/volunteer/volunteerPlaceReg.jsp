...
<script src="${pageContext.request.contextPath}/resources/static/js/volunteer/volunteerPlaceReg.js"></script>
  ...
  <form action="/service/regVolunteerPlace.do" method="post" id="regVolunteerForm">
    ...
    <div class="form-group">	
      ...
      <select name="cityCode" id="cityCode">
        ...
        <c:forEach var="mainCity" items="${mainCityList}">
	  		<option value="${mainCity.cityCode}">${mainCity.cityName}</option>
        </c:forEach>
      </select>
    </div>
    ...
    <div class="form-group">
      ...
      <div style="display: flex; gap: 10px; flex-wrap: wrap;">
        <input type="text" id="placePostCode" name="placePostCode" placeholder="우편번호" readonly>
        <input type="button" class="btn" id="getPostCode" value="우편번호 찾기">
      </div>
      <input type="text" id="placeAd" name="placeAd" placeholder="도로명주소" class="mt-2" readonly>
      <input type="text" id="placeAdd" name="placeAdd" placeholder="상세주소" class="mt-2">
    </div>
